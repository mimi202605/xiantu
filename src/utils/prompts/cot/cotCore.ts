...
### 基础同步（V3短路径）- 必须全面更新！
□ 位置变化 → set \`角色.位置\`（描述须为**具体**地点名，禁止「国外」「某城市」「某地点」「某办公室」「某区域」等泛指/占位；禁止「XX外」类相对指代如总统套房外、包厢外、门外，须为有明确名称的场所）
□ 时间流逝 → add \`元数据.时间.分钟\`
...
□ NPC属性变化（重要！）：
  ...
  - NPC位置变化 → set \`社交.关系.[NPC名].当前位置\`（描述须为**具体**地点名，禁止泛指/占位；禁止「XX外」类相对指代如总统套房外、包厢外、门外，须为有明确名称的场所）
