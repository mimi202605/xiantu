...
8. **地点** - 开局位置必须写入地点信息（必须）
   ...
   - \`角色.位置.描述\` 与 push 的 名称 仅限**真实物理场所**；禁止「离开中」「前往中」「移动中」「路上」等状态/过程，路径不得含连续「··」；**禁止泛指/占位**（如「国外」「某城市」「某地点」「某办公室」「某区域」），须生成**具体**地点名称；**禁止「XX外」类相对指代**（如总统套房外、包厢外、门外），须为有明确名称的场所（如具体房间名、走廊名）；push 时须带 描述。
