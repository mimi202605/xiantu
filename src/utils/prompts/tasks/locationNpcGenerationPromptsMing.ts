...
- **禁止非地点**：若当前地点为载具、飞行器或「XX内」（如豪华轿车内、飞舟内、机舱内、船内、车厢内），**不得** push \`世界.信息.地点信息\`；仅可输出 set NPC，且不得新增或补全此类非地点到地点信息。**禁止状态/过程**：不得将「离开中」「前往中」「移动中」「路上」等作为地点或 角色.位置；路径不得含连续「··」（如 区域··离开中）。
