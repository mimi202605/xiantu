...
- 天赋: [{名称, 描述}] (只读)
- 先天六司: {体质, 直觉, 悟性, 气运, 魅力, 心性} (只读，1-10，对修炼/战斗加成占比70%)
- 后天六司: {体质, 直觉, 悟性, 气运, 魅力, 心性} (可修改，用add，每项上限20，单次增加1-3点，对加成占比30%)
  - 获取方式：装备增幅、天赋效果、服用丹药、机缘奇遇(最多+5)、大道感悟、境界突破
  - 等价原则：先天1点 ≈ 后天2.33点效果

### 1.2 属性（Modifiable，路径：\`角色.属性\`）
- 境界?: {名称, 阶段, 当前进度, 下一级所需, 突破描述} (可选，偏修仙；用set更新)
- 声望: number (用add)
- 气血: {当前, 上限} (生命/体力，通用；当前用add，上限突破时用add)
- 灵气: {当前, 上限} (精力/能量，通用；当前用add，上限突破时用add)
- 神识?: {当前, 上限} (可选，偏修仙；精神力)
...
必需字段：名字/性别/出生日期/属性/与玩家关系/好感度/当前位置.描述/当前外貌状态/当前内心想法/记忆[]/背包/实时关注
禁止残缺对象：只给名字/好感度/当前状态视为错误
...
- 私密信息?: PrivacyProfile (NSFW模式专用；当 系统配置.nsfwMode=true 且 性别符合 系统配置.nsfwGenderFilter 时为必填，禁止“待生成/暂无/空对象”)

### 4.3 NPC创建示例（正确做法）
**假设当前游戏时间是仙道1年，NPC 18岁，则出生日期.年 = 1 - 18 = -17**
\`\`\`json
{"action":"set","key":"社交.关系.林婉儿","value":{
  ...
  "属性":{"气血":{"当前":1200,"上限":1200},"灵气":{"当前":1500,"上限":1500},"寿元上限":200},
  "与玩家关系":"同门师妹",
  "好感度":30,
  "人格底线":["不会背叛宗门","不会伤害无辜"],
  "记忆":["初次见面于宗门大殿"],
