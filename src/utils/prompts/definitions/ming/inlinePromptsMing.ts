...
## 地点与探索（必须）
- **地点仅限真实场所**：禁止将载具（轿车内、马车内）、飞行器（飞舟内、机舱内）、船内、车厢内等**非地点**作为 角色.位置.描述 或 push 到 世界.信息.地点信息；若角色在移动载具中，写其当前所处的地面地点（如出发地/目的地/途经地）。**禁止将状态/过程当地点**：不得使用「离开中」「前往中」「移动中」「路上」等作为位置或地点名称；路径不得含连续「··」（如 区域··离开中）。**禁止泛指/占位地点**：不得使用「国外」「某城市」「某地点」「某办公室」「某区域」等；须生成**具体**地点名称。**禁止「XX外」类相对指代**：不得使用「总统套房外」「包厢外」「门外」等作为地点或 角色.位置.描述；须为有明确名称的场所（如具体房间名、走廊名）。push 地点时须带 描述。
- **set 角色.位置 到新地点时，必须同时 push \`世界.信息.地点信息\`** 添加该地点
- **生成地点时必须生成其全部父级地点结构**：若地点名称为多级路径（如 \`S市·上城区·素家庄园\`），须按层级依次 push 每一级——先 push 根级（如 \`{名称:"S市", 描述?}\`），再 push 第二级（如 \`{名称:"S市·上城区", 上级:"S市", 描述?}\`），再 push 最末级（如 \`{名称:"S市·上城区·素家庄园", 上级:"S市·上城区", 描述?, 地点NPC?}\`）。缺漏父级会导致地图无法正确显示层级。
- 格式：push \`{名称: 与角色.位置.描述一致, 描述?, 上级?, 地点NPC?}\`；名称为全路径，\`上级\` 为父地点名称（上一级全路径）
