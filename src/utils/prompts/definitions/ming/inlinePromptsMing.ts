...
## 地点与探索（必须）
- **地点仅限真实场所**：禁止将载具（轿车内、马车内）、飞行器（飞舟内、机舱内）、船内、车厢内等**非地点**作为 角色.位置.描述 或 push 到 世界.信息.地点信息；若角色在移动载具中，写其当前所处的地面地点（如出发地/目的地/途经地）。**禁止将状态/过程当地点**：不得使用「离开中」「前往中」「移动中」「路上」等作为位置或地点名称；路径不得含连续「··」（如 区域··离开中）。**禁止泛指/占位地点**：不得使用「国外」「某城市」「某地点」「某办公室」「某区域」等；须生成**具体**地点名称；push 地点时须带 描述。
