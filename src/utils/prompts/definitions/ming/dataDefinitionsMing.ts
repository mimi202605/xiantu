...
- 位置: {描述:"大陆·区域·地点", x, y} (用 set 更新整个对象)
  - 描述: string (必填，格式"大陆·区域·地点"，**仅真实物理场所**；禁止状态/过程如「离开中」「前往中」「移动中」「路上」，路径不得含连续「··」；**禁止泛指/占位**：不得使用「国外」「某城市」「某地点」「某办公室」「某区域」等，须为**具体**地点名称；**禁止「XX外」类相对指代**：不得使用「总统套房外」「包厢外」「门外」等，须为有明确名称的场所如具体房间名、走廊名)
  - x: number (0-10000)
  - y: number (0-10000)
...
- 记忆: string[]
- 记忆总结: string[]
- 当前位置: {描述: string, x?, y?}（描述须为**具体**地点名，禁止「国外」「某城市」「某地点」「某办公室」等泛指/占位；禁止「XX外」类相对指代如总统套房外、包厢外、门外，须为有明确名称的场所）
- 当前外貌状态: string
- 当前内心想法: string
...
- **禁止泛指/占位**：不得使用「国外」「某城市」「某地点」「某办公室」「某区域」等泛指或占位名称；须生成**具体**地点名称（如具体城市、建筑、街道、区域名）。**禁止「XX外」类相对指代**：不得使用「总统套房外」「包厢外」「门外」等作为地点名称或 角色.位置.描述；须为有明确名称的场所（如具体房间名、走廊名）。push 地点时**须带 描述**（简短说明该场所），不得仅写 名称+上级 而无 描述。
