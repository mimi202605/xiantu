...
- 天赋: [{名称, 描述}] (只读)
- 先天六司?: {体质, 直觉, 悟性, 气运, 魅力, 心性} (只读，1-10，通用属性)
- 后天六司?: {体质, 直觉, 悟性, 气运, 魅力, 心性} (可修改，用 add，每项上限 20)
...
### 2.1 背包结构
- 金钱: {下品: number, 中品: number, 上品: number, 极品: number}（通用货币，用 add 增减；四档可对应世界观如铜钱/银两/金币等）
...
### 5.2 地点信息（递归结构）
路径: 世界.信息.地点信息
- 顶层地点并列存储；每项可有 内部 递归子地点
- 用 push 添加：{名称, 描述?, 上级?, 内部?, 地点NPC?}
  - 名称: string（唯一标识，与 当前位置.描述 匹配）
  - 描述?: string（简短说明）
  - 上级?: string（父地点名称，根节点无）
  - 内部?: LocationEntry[]（子地点数组，递归）
  - 地点NPC?: string[]（本地的 NPC 名列表；玩家离开后重点 NPC 会离开，普通 NPC 留守）
- 示例：push \`世界.信息.地点信息\` \`{名称:"百货大楼",描述:"繁华商区",内部:[{名称:"百货大楼一楼",上级:"百货大楼"}]}\`
