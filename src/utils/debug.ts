...
export class DebugLogger {
  ...
  private aiApiLogging: boolean = false;
  ...
  private loadDebugSettings(): void {
    try {
      ...
      if (settings) {
        ...
        this.aiApiLogging = parsedSettings.aiApiLogging || false;
      }
    } catch (error) {
   ...
   * 获取AI API日志状态
   */
  public isAiApiLoggingEnabled(): boolean {
    return this.debugMode && this.aiApiLogging;
  }
  ...
  /**
   * AI API 请求日志 - 只在启用AI API日志时显示
   */
  public logAiRequest(flow: string, requestId: string, prompts: any[], usageType?: string): void {
    if (this.isAiApiLoggingEnabled()) {
      console.group(`[AI API - ${flow}] 请求 #${requestId}${usageType ? ` (${usageType})` : ''}`);
      console.log('📤 发送的Prompts:');
      prompts.forEach((prompt, index) => {
        console.log(`  ${index + 1}. [${prompt.role}] ${prompt.content.substring(0, 200)}${prompt.content.length > 200 ? '...' : ''}`);
        if (this.isConsoleDebugEnabled()) {
          console.log('    完整内容:', prompt.content);
        }
      });
      console.groupEnd();
    }
  ...
  /**
   * AI API 响应日志 - 只在启用AI API日志时显示
   */
  public logAiResponse(flow: string, requestId: string, response: string, usageType?: string): void {
    if (this.isAiApiLoggingEnabled()) {
      console.group(`[AI API - ${flow}] 响应 #${requestId}${usageType ? ` (${usageType})` : ''}`);
      console.log('📥 接收的响应:');
  ...
  /**
   * AI API 错误日志 - 只在启用AI API日志时显示
   */
  public logAiError(flow: string, requestId: string, error: any, usageType?: string): void {
    if (this.isAiApiLoggingEnabled()) {
      console.group(`[AI API - ${flow}] 错误 #${requestId}${usageType ? ` (${usageType})` : ''}`);
...
export const debug = {
  ...
  // AI API 日志方法
  logAiRequest: (flow: string, requestId: string, prompts: any[], usageType?: string) => debugLogger.logAiRequest(flow, requestId, prompts, usageType),
  logAiResponse: (flow: string, requestId: string, response: string, usageType?: string) => debugLogger.logAiResponse(flow, requestId, response, usageType),
  logAiError: (flow: string, requestId: string, error: any, usageType?: string) => debugLogger.logAiError(flow, requestId, error, usageType)
};
