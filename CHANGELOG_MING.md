...
- **问题**：酒馆模式下主游戏流程（`main`）的 API 分配被 UI 硬锁，无法切换到独立 API，即使后端路由逻辑本已支持。
- **修改**
  - **`APIManagementPanel.vue`**：移除 `main` 的 `tavern-locked` 样式、`🔒 酒馆API` 角标及静态 `使用酒馆配置` 文字，改为与其他功能一致的 `<select>` 下拉选择器；酒馆模式下 `default` 选项显示为 `🍺 酒馆API`。
  - **描述文字**：模式说明与功能描述更新，准确反映「默认走酒馆API，可分配独立API绕过酒馆直连」的行为。
  - **注释同步**：`aiService.ts`、`apiManagementStore.ts` 中的架构注释去掉「main 永远走 TavernHelper」的错误表述。
- **行为**：`main` 分配 `default` → 走酒馆 TavernHelper（原有行为不变）；分配独立 API → 绕过酒馆直连该 API（后端路由逻辑无需修改，本已支持）。
...
- **游戏版本号**：更新为 1.0.6（`package.json`）；设置导出中的 `exportInfo.version` 与 `gameVersion` 显示为「天命 v1.0.6」（`SettingsPanel.vue`）。
...
- **网站标题**：`index.html` 中 `<title>` 由「仙途」改为「天命」，浏览器标签页显示「天命」。
- **游戏版本号**：更新为 1.0.5（`package.json`）；设置导出中的 `exportInfo.version` 与 `gameVersion` 显示为「天命 v1.0.5」（`SettingsPanel.vue`）。
