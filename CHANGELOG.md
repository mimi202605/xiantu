...
## [MING] 通用版 0.3.1 (2026-02-26)

- **地点**：禁止「XX外」类相对指代（总统套房外、包厢外、门外等），须为有明确名称的场所；规则覆盖数据结构、业务规则、CoT、地点 NPC 生成、世界心跳、角色初始化等提示词。
- **探索地图**：当前位置改为单链高亮（当前节点到根路径），不高亮兄弟节点。
  详见 **[CHANGELOG_MING.md](./CHANGELOG_MING.md)**。

---

## [MING] 通用版 1.0.7 (2026-02-26)

- **游戏版本**：更新为 1.0.7（界面与设置导出版本号「天命 v1.0.7」）。
- **行动选项**：新增选项模式（行动导向 / 剧情导向）与节奏（快速推动 / 慢速体验）；剧情导向生成剧情级叙述选项（背景、人物逻辑、走向），禁止分条格式；选项数量 3–5 个不可少给；持久化与导出/导入支持。
- **主界面输入框**：随内容动态增高（上限 200px），超出后显示垂直滚动条；程序填入内容时同步调整高度；滚动条样式改为可见。
  详见 **[CHANGELOG_MING.md](./CHANGELOG_MING.md)**。

---

## [MING] 通用版 0.3.0 (2026-02-26)
