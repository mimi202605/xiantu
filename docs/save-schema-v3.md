...
└─ 任务统计: object
   ├─ 完成总数: number
   └─ 各类型完成: object (key=QuestType -> number)
...
├─ 任务状态?: string
├─ 目标列表: array
│  └─ [i]: object { 描述, 类型, 目标ID, 需求数量, 当前进度, 已完成 }
├─ 奖励: object (可选字段组合)
├─ 创建时间: object|string
├─ 完成时间?: object|string
├─ 有效期分钟?: number
